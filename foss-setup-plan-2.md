...
- **Encrypt the DNS that leaves your house.** Filtering alone still forwards plaintext queries your ISP can read. Either point Pi-hole/AdGuard at **Unbound** as a local **recursive resolver with DNSSEC** (no upstream sees your queries at all), or at minimum enable **DoT/DoH** to Quad9/Cloudflare.
- **Fail-open DNS chain (required before AdGuard-only DHCP):** DHCP must list **three** resolvers — primary AdGuard on the Mac mini, **secondary AdGuard on the NAS** (independent DoT upstream, not mini Unbound), and **tertiary UniFi gateway** for bare internet when both filter boxes are down. A single-DNS deployment is a house-wide outage when the mini reboots (incident 2026-07-03). See `foss-setup/configs/network/dns-resilience-plan.md` and rollout tasks **dns-02 → dns-05**.
- **Anti-bypass (optional, after fail-open is proven):** NAT-redirect outbound port 53 to your resolver and **block known DoH endpoints** — only enable after the three-tier chain is verified; otherwise a mini outage recreates the single-point-of-failure.
...
>   Finished Soulseek files reach the NAS via the same rclone mount
>   `/volume1/mounts/seedbox-files` used by the Deluge pipeline.
...
> *Superseded design:* the original seedbox-hosted pipeline that the box above replaces (qBittorrent + the full \*arr suite + a sync agent all on the seedbox) has been moved to [Appendix A — superseded designs (kept for history)](#appendix-a--superseded-designs-kept-for-history) at the end of this file.
...
- **Decision — Caddy owns 80/443 on the Mac mini.** Caddy is the single reverse proxy for the whole hand-managed stack: it reaches each service by container name over a shared `edge` Docker network (`reverse_proxy seerr:5055`), so it *must* be co-located with those containers. Note that container-name routing only covers services on the mini's `edge` network — NAS and rig services are proxied **by IP** via the `NAS_IP`/`RIG_IP` env vars in `caddy/.env`, not by container name. That rules out a second proxy also binding 80/443 on the same box. **Coolify (next) ships its own bundled proxy that wants 80/443 too** — so do *not* let Coolify's proxy take those ports here. Two ways to keep the peace, in order of simplicity:
...
- **Dependency-Track v5 (codename "Hyades")** — the open-source platform (20,000+ orgs) that ingests **CycloneDX** SBOMs and **continuously re-analyzes your entire inventory** against multiple vulnerability sources (NVD, OSS Index, GitHub advisories) with **EPSS** prioritization — not just at scan time, but every time new intel lands. It tracks **OS packages, containers, firmware, and services** across every version of every "project," which maps perfectly onto "every host and every stack in my house."
  ...
  - *Host on:* the **NAS** (always-on, with RAM headroom — the Java API server wants **~4GB+** on its own (official minimum 2-4.5GB, recommended 8-16GB), which is exactly the weight Section 0's capacity note keeps off the small Mac mini). **This requires the DS920+ RAM upgrade first** (4GB → 20GB via the Crucial `CT16G4SFD8266`, ~$40-110, decided/future purchase — see Section 0's capacity note); until that upgrade, **Dependency-Track runs on the Mac mini, where it is deployed today** — Section 0's limp-mode rule applies only to the NAS. (The nightly Syft job still PUTs SBOMs to it over the LAN/Tailscale regardless of host.)
...
3. **Lock in backups:** Synology Hyper Backup + Snapshot Replication, plus a Restic/Kopia job for your irreplaceable data -> B2. Test one restore. **When reorganizing NAS shares across the three Basic volumes, follow [foss-setup/configs/nas/SCHEMA.md](foss-setup/configs/nas/SCHEMA.md)** — its runbook depends on this safety net (Tailscale + B2) being in place first.

**Phase 2 — De-cloud the essentials**

3a. **Prerequisite — prep the Mac mini Docker host.** Before the first stack here (LiteLLM, Seerr) comes up, install Docker and **create the shared proxy network once: `docker network create edge`**. Every web stack joins `edge` as an `external` network — `docker compose up` fails if it doesn't exist yet — and Caddy (Phase 4) later reaches each service by container name across it. (See `foss-setup/configs/docker-stack/README.md` → *One-time host prep*.) *(Numbered 3a so the existing step numbers 4–20 stay stable.)*
...
> Moved here 2026-07-07 from Section 2 (*Media acquisition*). The sections below describe the **original seedbox-hosted pipeline** (qBittorrent + the full \*arr suite + a sync agent on the seedbox) and are kept for history only; where they conflict with the ⭐ ARCHITECTURE UPDATE box in Section 2, **that box wins.**
