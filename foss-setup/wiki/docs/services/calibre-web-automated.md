...
- **The container shows `(unhealthy)` (FailingStreak climbing) but the app serves login fine, or `docker logs calibre-web-automated` hangs for minutes.** — Both were fix-55. The fork image's built-in healthcheck curls `/` with a 3s timeout; under NAS I/O load that probe intermittently exceeds 3s and the container flips `unhealthy` (SM10) while `/login` still answers in ~25ms — the compose file (live + repo mirror) now overrides it with a lighter `/login` probe on a 60s interval / 20s timeout / 5 retries / 120s start_period. Separately, Synology's default `db` log driver made `docker logs` hang for minutes under load (SM31); the compose now pins `logging: json-file` with `max-size: 10m` / `max-file: 3`, so `docker logs` returns instantly and the log is bounded. If either regresses after an edit, restore those two blocks and `docker compose up -d`; note CWA re-runs its ~50s calibre-binaries setup on every recreate before the web server binds (the `xdg-desktop-menu ... exit 3` line during that setup is a benign headless warning).
