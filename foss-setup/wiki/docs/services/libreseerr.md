...
Libreseerr is the book-request portal in Brandon's *arr media stack — the ebook/audiobook counterpart to Seerr (movies/TV) and MusicSeerr (music) — running as a single `ghcr.io/zamnzim/libreseerr` container on the Mac mini (`/opt/stacks/libreseerr`), exposed on `8789:5000` over the external `edge` (Caddy/Tailscale) network at https://libreseerr.tabaska.us. Since bmig-04 (2026-07-20) it fronts Bookshelf on the NAS (`http://192.168.10.4:8790`, hardcover metadata via rreading-glasses-hc) to turn user book requests into monitored+searched additions, and pulls discovery/browse data from OpenLibrary (with an explicit `fields=` list — OL's default search fields stopped including `isbn`, which would silently disable the ISBN-first lookup). Three files are bind-mounted read-only as a local patch — `./app.py`, `./readarr.py`, and `./bookshelf.py` (a `ReadarrClient` subclass that shadows the image's unpatched Bookshelf client; the fork speaks the same v1 API) — that re-rank lookup candidates (upstream blindly picks `readarr_books[0]`), enforce the bmig-04 author-verification gate (a candidate's OWN author must token-match the requested author or be a bio-cross-referenced pen name like Mira Grant ↔ Seanan McGuire; authorless candidates are never stamped with the requested author), prefer ISBN lookups over term search, and fail loudly to ntfy `books` when no eligible candidate exists instead of adding the next-best wrong book (C1/C4). The same patched app.py also runs a background status reconciler (fix-26) that keeps the request dashboard truthful against the backend instead of only refreshing on UI opens. The image is pinned by MANIFEST digest (`820134e4`), not by local image ID, because upstream publishes no version tags and garbage-collects old digests.
...
| `libreseerr` | `./bookshelf.py:/app/bookshelf.py:ro` |
...
- **Requests are accepted in Libreseerr but nothing downloads.** — Libreseerr only hands off to Bookshelf on the NAS (http://192.168.10.4:8790 since bmig-04). Check Bookshelf itself — confirm the book is monitored and searched, and that the indexer/download-client chain is healthy. Verify connectivity from the mini container: logs should show `GET /api/v1/book HTTP/1.1 200`.
- **A request errors "No eligible metadata candidate … refusing to add a different book (author gate)" (ntfy push), but the book looks legitimate.** — The bmig-04 author gate refused every lookup candidate: none had an author token-matching the requested author (pen names pass only when the two hardcover author bios name each other in BOTH directions — the Mira Grant ↔ Seanan McGuire shape). This is the designed correct-or-loud behavior replacing the C1/C4 silent wrong-book adds ('Pride, Prejudice, and Peril' bound to a Jane Austen request). Check the rejected-candidates list in the error: if the author spelling differs between OpenLibrary and hardcover, re-request under the name hardcover uses; if hardcover genuinely lacks the work, it cannot be added automatically — grab it manually via Bookshelf interactive search.
...
- **A request errored "author unresolvable" (or "not found in the backend metadata provider") for a book/author that IS in Hardcover and resolves fine on a re-probe minutes later.** — fix-57 SM5 (2026-08-02): rreading-glasses (Hardcover mode) fires a background full-catalog refresh the first time any author is touched, and while it runs `/author/lookup` and `/book/lookup` intermittently return `[]` or a transient 429/5xx even for warm records (the documented ~60/min quota + thundering-herd). readarr.py's `resolve_author`/`search_books` used to take a single empty response as PERMANENT — 'Le Morte d'Arthur' by Thomas Malory errored "author unresolvable" on 07-28 while the author resolved on the first try on 08-02. Both lookups now retry empty/transient results via `_retry_empty` (LOOKUP_RETRIES=3, LOOKUP_RETRY_BACKOFF=2s, genuine non-429 4xx stay permanent). If it still errors after the retries, the backend really is empty — re-request once the refresh storm settles. The `libreseerr-request-flow-health` check flags any NEW mechanism/transient-class request error (author-unresolvable/timeout/5xx) since the fix — a recoverable book that silently failed — while deliberately NOT alarming on the correct guard refusals below.
- **A batch of PUBLIC-DOMAIN CLASSICS (Homer, Emily Brontë, Jane Austen, Malory, Arthurian romances) all error "No eligible metadata candidate" / "not found in Hardcover" — many errors, last success weeks ago.** — fix-57 SM5 (2026-08-02): this is NOT a broken pipeline — it is a genuine Hardcover metadata-coverage gap and the fix-48 guard correctly refusing to add a degraded record. Hardcover (a modern social-reading DB) mis-carries public-domain classics: a search for 'Wuthering Heights' returns CliffsNotes / annotated study editions / a Vietnamese edition ("TH'inh gio hu") but not the plain canonical work; 'The Odyssey' comes back under the author 'homer' while the request carried the Greek form 'Όμηρος' (author-gate mismatch); 'Persuasion' returns 'Persuasion, The Coloring Book' (derivative-token reject); 'Rolling in the Deep' is a Mira-Grant pen-name novella Hardcover doesn't surface for 'Seanan McGuire'. Results are also nondeterministic under refresh-herd. Do NOT force these in (that re-introduces the C1/C4 degraded-record class). Options: re-request under the exact author-name form Hardcover uses, or acquire the ebook manually via Bookshelf interactive search / a public-domain source (Standard Ebooks, Project Gutenberg) dropped into the CWA ingest folder.
