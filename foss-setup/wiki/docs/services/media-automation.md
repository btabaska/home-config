...
The full home `*arr` acquisition stack, running as the `media-automation` Compose project on the NAS (DS920+, Container Manager) and defined in `foss-setup/configs/nas/media-automation/docker-compose.yml`. It bundles `prowlarr` (indexer manager, with `flaresolverr` for Cloudflare) feeding `sonarr` (TV→`/volume3` `/tv`), `radarr` (movies→`/volume2` `/movies`), `lidarr` (music→`/volume1` `/music`), and `bookshelf` (books→`/readarr-library` — the maintained Readarr fork, :8790, `ghcr.io/pennydreadful/bookshelf` hardcover variant, digest-pinned, API key at vault `arr_api_keys.bookshelf`); `unpackerr` extracts archives (its books slot speaks the readarr protocol via `UN_READARR_0_*` env pointed at bookshelf), `soularr` bridges Lidarr's wants to slskd on the seedbox, and `beets` is an on-demand tag-only layer. Books metadata comes from the self-hosted `rreading-glasses-hc` (:8789, Hardcover mode, database `rreading_glasses_hc` in the shared `rreading-glasses-db` postgres:17.6 — superuser role `rreading-glasses`, NOT `postgres`; token at vault `books.hardcover_api_token`, **expires every Jan 1**, guarded by the `hardcover-token-valid` check). The goodreads-era pair (EOL readarr :8787 + goodreads-mode rreading-glasses :8788) was decommissioned in the books cutover (`docs/books-metadata-cutover-2026-07-20.md`, bmig-01…06, completed 2026-07-20): containers removed in bmig-05, the 14GB goodreads database dropped in bmig-06 (pgdump archived at `/volume1/archive/books-cutover-bmig05/`). Bookshelf mirrors the readarr-era policy: EPUB-Preferred quality profile (the stock PDF-tolerant `eBook` profile is deleted — bmig-06 — and libreseerr's patched client orders EPUB Preferred first so it stays the request default), foreign-language + companion/coloring-book release blocklist, Deluge client with its OWN `bookshelf`/`bookshelf-imported` categories, remote path mapping, and the CWA-ingest Connect script (note: Bookshelf ManualImport DOES fire Connect, unlike readarr). Beware: POSTing a root folder triggers an immediate disk scan that auto-adopts matched files — under Hardcover quota pressure (~60 req/min account-wide) this mints junk/duplicate records; searches also degrade while author refreshes storm. The two halves connect off-site: home apps drive the seedbox "Betty" Deluge over its API via the tailnet (`100.119.134.94:5945` — since fix-21 Deluge binds loopback on Betty and the public IP exposes nothing; the NAS Tailscale package must stay in TUN mode) and READ completed downloads through an rclone SFTP mount (`seedbox:/home/hd34/btabaska/files` → `/volume1/mounts/seedbox-files`), which every download-touching container binds at the identical path `/seedbox`; each `*arr` needs a Remote Path Mapping (`/home/hd34/btabaska/files/` → `/seedbox/`) so Deluge-reported paths resolve inside the container. The critical DSM constraint is that `fuse.rclone` cannot satisfy `:rslave` propagation, so the seedbox is a plain bind and download-touching containers must be restarted after any watchdog remount; imports feed downstream to Plex, the CWA book pipeline, and the iPod/Rhythmbox music sync.
...
- **MULTIPLE arrs log 'database is locked' at once, scheduled tasks (RefreshMonitoredDownloads, ImportListSync) fail, APIs 500, and soularr fatal-exits every cycle — `arr-sqlite-not-locked` / `nas-io-pressure` firing.** — This is fix-55, and it is NOT per-app corruption — it is NAS-wide I/O saturation. Under sustained disk pressure the arrs' SQLite fsync runs slower than their busy_timeout, so writers hit SQLITE_BUSY ('database is locked') and 500 their own tasks + callers (musicseerr, soularr). The arrs expose NO busy_timeout knob (config.xml carries only LogLevel) and their DB files are already `nodatacow` (+C), so the fix is to REMOVE the I/O pressure, not tune SQLite: check `nas-io-pressure` (15-min IO-load; storm sat at 17-22 vs ~2.8 quiet) and find the hog with `sudo sh -c 'for p in /proc/[0-9]*; do ...read_bytes/write_bytes...; done'` — the usual suspect is bitmagnet's DHT crawler (throttled to `DHT_CRAWLER_SCALING_FACTOR=3` in fix-55; confirm it did not creep back to 10) plus btrfs-cleaner GC and unbounded Synology `db` docker-logs. soularr does not truly crash-loop: `restart: always` + the image's own wrapper re-run it, and the per-run 'Fatal error! Exiting' is upstream image behaviour (no retry knob); fix-55 lengthened its `SCRIPT_INTERVAL` 300->900 to cut the failed-cycle churn, and eliminating the arr 500s is what actually stops the fatal-exits. Full remediation + measurements: `wiki/docs/runbooks/nas-host-hygiene.md`.
- **soularr grinds every cycle on the same albums (`Skipping failed import album` for a growing list; `N releases failed to find a match and are still wanted`), or `nas-soularr-failed-imports-fresh` / `lidarr-incomplete-albums` fire — the music wanted list never drains.** — This is fix-56 (fleet-sweep SM7/SM29/SM30), and there are three distinct rots to keep apart. (1) DENYLIST DEAD-LETTER: soularr records every failed import in `nas:/volume1/docker/soularr/failed_imports.json` and skips it forever, but the upstream image (`add_to_failed_import_denylist` in `/app/soularr.py`) NEVER removes an entry — so the file collects ghosts (albums completed via a torrent/manual grab) and abandoned (unmonitored) entries until an entry ages past the 3-day `nas-soularr-failed-imports-fresh` threshold. The closer is the `soularr-denylist-reconcile` mini timer (configs/host/mini/soularr-reconcile, every 6h) which prunes any entry whose album is complete/unmonitored/deleted in Lidarr; `soularr-denylist-no-ghosts` is its outcome probe. Do NOT hand-edit the JSON — `systemctl start soularr-denylist-reconcile` on the mini. (2) JUNK MUSICBRAINZ RELEASES: bootleg/mislabeled release-groups (e.g. Chinese-titled 'albums' under a Western pop artist) get monitored when an artist is added and soularr searches them every cycle, never finding a match. Unmonitor them in Lidarr (`PUT /api/v1/album/monitor {albumIds, monitored:false}`) — they drop out of wanted/missing so soularr stops grinding; the on-disk monitored flag persists across metadata refreshes. (3) STUCK PARTIALS: `album_match` requires ALL tracks before grabbing, so a partial album (e.g. 17/24 on disk) came from a flaky Soulseek transfer, not a config knob — either complete it out-of-band (a Lidarr AlbumSearch on the torrent path) or unmonitor it (files stay on disk, `lidarr-incomplete-albums` only flags MONITORED partials). Completed-but-importFailed grabs stuck in the Lidarr queue ('One or more tracks expected ... were not imported' when the album is already 100% on disk) are now auto-reaped by the `arr-queue-reconcile` timer (Lidarr added in fix-56). Full write-up: `wiki/docs/runbooks/nas-host-hygiene.md`.
