...
- **seerr-request-rot check fails (SEERR_ROT N): requests stuck PROCESSING for weeks — "dangling" (the mapped Sonarr/Radarr series/movie was deleted after the request) or "never-grabbed" (>7d old, zero arr history)** — The background status reconciler only advances requests whose backend still exists; it deliberately does NOT auto-delete orphaned/never-grabbed requests. Reconcile manually (fix-90 pattern): identify them with `python3 /opt/verification/bin/request-layer-audit.py seerr`, then for each purge the stuck record via the API (re-requestable if still wanted, and this does NOT re-download removed content, unlike a retry): `KEY=$(python3 -c 'import json;print(json.load(open("/opt/stacks/seerr/config/settings.json"))["main"]["apiKey"])'); curl -X DELETE -H "X-Api-Key: $KEY" http://192.168.10.2:5055/api/v1/request/<id>` (expect 204). Re-run the audit → SEERR_OK. Prefer DELETE over retry for dangling TV — a retry re-adds the deleted series to Sonarr and can re-download a whole season.
