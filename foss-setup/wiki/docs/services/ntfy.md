...
- **A unit fails on mini but no ntfy page arrives (silent OnFailure), and/or a secret leaks into the journal.** — `OnFailure=ntfy-notify@%n.service` runs `/opt/scripts/ntfy-notify.sh`, which sources `/etc/verification/env` under `set -euo pipefail` and posts to the `backups` topic. If ANY value in that env file is unquoted-with-a-space or line-wrapped, the shell executes it as a command (exit 127): the wrapper dies before publishing (so EVERY mini OnFailure page is silently dropped) and the raw value is written to journald. This was sec-12 (finding SH3): line 40's `HARDCOVER_API_TOKEN=Bearer <jwt>` split on the space. Fix = quote every space/metachar-bearing value onto one line — `KEY="Bearer eyJ..."`. Verify with `ssh mini 'sudo bash -c "set -a; . /etc/verification/env; set +a"'` (must exit 0, print nothing). Guarded by the `verification-env-source-clean` (sources clean, no leak) and `mini-onfailure-ntfy-delivers` (publish+read-back on the topic) checks; a synthetic OnFailure page must land on `backups`.
