...
> Per-title feasibility and host-assignment reference for hosting private game servers on the homelab fleet.
_Source: `foss-setup/configs/gaming/server-guide.md` · migrated + validated 2026-07-14._

A per-title feasibility reference for hosting private servers on the fleet defined in `foss-setup-plan-2.md` (repo root) Section 4. Use this before committing to a host or uptime model.
...
!!! note "Current live state (validated 2026-07-14)"
    Three game-adjacent services are already deployed 24/7 on the **CachyOS rig** and confirmed running:

    - **Palworld** — `ghcr.io/thijsvanloef/palworld-server-docker` (healthy)
    - **AMP** — `ghcr.io/imagegenius/amp` (hosts the MinecraftCross world)
    - **playit** — `ghcr.io/playit-cloud/playit-agent` (public tunnel)

    The **Mac mini** runs `bedrock-connect` (a lightweight Bedrock redirect helper for crossplay), not a full world server — so the mini's "one game slot" is still open. Everything else below remains a feasibility plan.
...
Grounded in `foss-setup-plan-2.md` Sections 0, 4, and 5.
...
Power: rig runs 24/7 (decision 2026-07-08 — ~130W idle ≈ $23/mo accepted for
  availability; idle-power-tuning task open). WoL is retained as RECOVERY
  tooling (power outage / accidental shutdown), not part of the workflow.
```
...
From `foss-setup-plan-2.md` Sections 1, 4, and 7.
...
| **24/7 (rig)** | CachyOS rig — runs 24/7 since 2026-07-08 (WoL is recovery-only) |
...
| **RuneScape: Dragonwilds** | Yes | Marginal (≤4p) | Marginal | Yes | **Mac mini** (≤4p) or **CachyOS** (6p) | Both | 24/7 (either host) |
| **Team Fortress 2** | Yes | Marginal | No | Yes | **Mac mini** (vanilla) or **CachyOS** (modded) | Both | 24/7 (either host) |
| **Unreal Tournament 2004** | Yes | Yes | No | Yes | **Mac mini** — lightest 24/7 pick | Both | 24/7 |
| **Halo: CE Custom Edition** | Yes | Marginal | No | Yes | **Mac mini** (Wine) or **CachyOS** | Both | 24/7 (either host) |
| **Core Keeper** | Yes | No | No | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
| **Valheim** | Yes | No | Marginal | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
| **Palworld** | Yes | No | No | Yes | **CachyOS** (24/7 rig) — *live* | Both | 24/7 (rig) |
| **Astroneer** | Yes (Wine) | No | No | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
| **Abiotic Factor** | Yes (Wine) | No | No | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
| **Final Fantasy XI** | Yes (emulator) | No | Marginal | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
| **WoW Vanilla 1.12.1** | Yes (emulator) | No | Marginal | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
| **WoW WotLK 3.3.5a** | Yes (emulator) | No | Marginal | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
| **Enshrouded** | Yes (Wine) | No | No | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
...
| **Game-night / heavy servers** | CachyOS (24/7) | Valheim, Core Keeper, Palworld, Enshrouded, Astroneer, Abiotic Factor, FFXI, both WoW emulators, modded TF2 |
...
flowchart TD
    ...
    q1 -->|No| rig[CachyOS rig — 24/7 heavy host]
...
- **Recommend:** **Mac mini** 24/7 (plan's default always-on slot). Point Paper's world directory at a NAS share for Hyper Backup coverage — run the JVM on the mini, store data on the NAS. *(Note: crossplay is currently served by AMP MinecraftCross on the rig + `bedrock-connect` on the mini; the mini's native world slot remains open.)*
- **Reach:** Both
- **Uptime:** 24/7
...
- **Reach:** Both
- **Uptime:** 24/7 (either host)
- **Resource estimate:** **2 GB + 1 GB per player** (Jagex formula); max 6 players = 8 GB; UDP **7777** (7778 for second instance)
- **Stacking vs existing load:** 2-player server (4 GB) is marginal on the Mac mini alongside the Docker stack — monitor with Beszel. 6-player (8 GB) belongs on the rig even though 64 GB makes it trivial there; the rig is 24/7, so either host gives always-on uptime.
...
- **Recommend:** **CachyOS** (24/7 rig) — needs Xvfb/OpenGL; plan Section 4 lists this on the rig.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Gotchas:** Unity engine requires a virtual display. Listed in main plan Section 4 as a rig co-op title. Modded or heavily explored worlds want 8 GB+.
- **Implementation priority:** Tier 2
...
- **Recommend:** **CachyOS** (24/7 rig). NAS Container Manager can run Valheim in theory, but J4125 lag under Immich/Frigate makes it a bad fit — store `worlds/` on NAS, run server on rig.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Verdict:** Yes — **currently live on the rig** (`thijsvanloef/palworld-server-docker`, 24/7)
- **Can host on:** Mac mini **No** · NAS (20 GB) **No** · CachyOS **Yes**
- **Recommend:** **CachyOS** only (24/7 rig) — CPU- and RAM-heavy; plan Section 4 explicitly routes Palworld to the rig. This is deployed today.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Tooling:** LinuxGSM; SteamCMD App ID **2394010**; community Docker (`jammsen/docker-palworld-dedicated-server`, `thijsvanloef/palworld-server-docker` — the deployed image). Compose lives at `foss-setup/configs/gaming/palworld/compose.yaml`.
...
- **Implementation priority:** Tier 2 — done
...
- **Can host on:** Mac mini **No** · NAS (20 GB) **No** · CachyOS **Yes**
- **Recommend:** **CachyOS** (24/7 rig) — Wine path; NAS has no Wine layer.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Verdict:** Yes (Wine — no native Linux server)
- **Can host on:** Mac mini **No** · NAS (20 GB) **No** · CachyOS **Yes**
- **Recommend:** **CachyOS** (24/7 rig) — UE5 under Wine; wrong box for mini or NAS.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Can host on:** Mac mini **No** · NAS (20 GB) **Marginal** · CachyOS **Yes**
- **Recommend:** **CachyOS** (24/7 rig). LandSandBoat Docker on NAS is technically possible at 20 GB but stacks MariaDB + four processes atop Paperless/DT/Immich — not worth the NAS risk.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Can host on:** Mac mini **No** · NAS (20 GB) **Marginal** · CachyOS **Yes**
- **Recommend:** **CachyOS** (24/7 rig). VMaNGOS wants compile + MariaDB; 8 GB Mac mini cannot hold it alongside the Docker stack; NAS adds Java/DB contention with Dependency-Track.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Can host on:** Mac mini **No** · NAS (20 GB) **Marginal** · CachyOS **Yes**
- **Recommend:** **CachyOS** (24/7 rig). `acore-docker` fits the rig's 64 GB; on NAS you'd pile another Java worldserver next to Paperless Tika and DT apiserver.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Verdict:** Yes (Wine — no native Linux server)
- **Can host on:** Mac mini **No** · NAS (20 GB) **No** · CachyOS **Yes**
- **Recommend:** **CachyOS** only (24/7 rig) — 16–24 GB real-world RAM and Wine leaks; nowhere else in the fleet.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
- **Stacking vs existing load:** Official docs claim ~4.4 GB idle + 100 MB/player, but community reports show **11–14 GB after 50 hours** of world building due to voxel chunk loading. With 64 GB rig this is fine. Wine leaks GDI handles → **daily restart** (`RuntimeMaxSec=86400` in systemd).
...
Aligned with `foss-setup-plan-2.md` Phase 5.
...
3. On the **CachyOS rig** (24/7 since 2026-07-08), install **Valheim** via LinuxGSM as the first heavy server.
...
5. Add **Core Keeper** and **Palworld** on the rig (LinuxGSM or Pelican). Set systemd timers for Palworld daily restart. *(Palworld is already deployed via Docker Compose.)*
6. If the group wants survival-craft variety, add **Astroneer** or **Abiotic Factor** (Wine path — test encryption/settings first).
7. Wire **Restic** (Section 6) to back up world/save folders on the nightly schedule (the rig is 24/7).
...
From `foss-setup-plan-2.md` Section 4.
...
| **Playit.gg** | Game tunnel if a friend cannot install Tailscale (playit agent runs on the rig) |
| **Beszel** | Monitor RAM/CPU on Mac mini and rig — watch for OOM on the mini |
| **Restic → B2** | Back up world saves (Section 6) on the nightly timer |
...
### Wake-on-LAN (recovery tooling only)

The rig is 24/7 since 2026-07-08 — WoL is no longer part of the game-night workflow. Keep it working as RECOVERY tooling for a power outage or accidental shutdown (rig down = incident):

1. Keep WoL enabled in BIOS on the CachyOS rig (`foss-setup/scripts/gaming/enable-wol-cachyos.sh`).
2. To recover a downed rig, send a magic packet from phone/laptop, HA automation, or SSH: `wol <mac-address>` (or `foss-setup/scripts/gaming/wake-rig.sh`).
3. Game servers restart via systemd/LinuxGSM (or Docker `restart: unless-stopped`) after the recovery boot.
...
*Cross-reference: `foss-setup-plan-2.md` (repo root) Section 4 (Game servers and game streaming) and Phase 5 item 18.*
