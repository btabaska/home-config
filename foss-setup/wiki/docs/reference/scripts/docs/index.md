...
`foss-setup/scripts/docs/` — 7 script(s).

| Script | Role |
|---|---|
| [`build-wiki.sh`](build-wiki-sh.md) | build and deploy wiki.tabaska.us (wiki-04) |
| [`gen-checks-pages.py`](gen-checks-pages-py.md) | wiki reference for every verification check (wiki build-out). |
| [`gen-roadmap-pages.py`](gen-roadmap-pages-py.md) | mirror the task tracker into the wiki (todo list). |
