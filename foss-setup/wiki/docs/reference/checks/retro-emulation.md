...
```bash
command -v es-de >/dev/null 2>&1 || { echo NO-ESDE; exit 1; }; ls /home/btabaska/ROMs/nes/ >/dev/null 2>&1 || { echo NAS-LIBRARY-UNREACHABLE; exit 1; }; for core in mesen snes9x mgba gambatte mupen64plus_next dolphin; do [ -e "/usr/lib/libretro/${core}_libretro.so" ] || { echo "MISSING-CORE:${core}"; exit 1; }; done; n=0; for s in gb gba n64 nes snes gc wii; do c=$(ls "/home/btabaska/ROMs/$s" 2>/dev/null | grep -vi eaDir | wc -l); n=$((n+c)); done; if [ "$n" -ge 8000 ]; then echo "ESDE-ROMM-OK roms=$n"; else echo "LIBRARY-THIN roms=$n"; exit 1; fi
```
