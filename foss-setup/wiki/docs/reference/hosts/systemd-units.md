...
| `music-mirror.service` | rig | oneshot: mirror NAS music → `~/Music` (`rsync -rt --delete-after`, ro automount `/mnt/nas-music-ro/`) for Rhythmbox/iPod; healthchecks `music-mirror-rig`; `TimeoutStartSec=30min`, `Nice=15`. See `reference/hosts/rig-timers.md` | (via timer) | `foss-setup/configs/host/rig/music-mirror/music-mirror.service` |
| `music-mirror.timer` | rig | daily music mirror | `*-*-* 05:00:00`, `RandomizedDelaySec=300`, `Persistent=true`. See `reference/hosts/rig-timers.md` | `foss-setup/configs/host/rig/music-mirror/music-mirror.timer` |
| `nas-music-mirror.service` | rig | oneshot: mirror NAS music → `~/Music` as iPod-playable ALAC/MP3 (`/home/btabaska/bin/nas-music-to-alac-mirror.sh`); `Nice=10`, IO class idle. See `reference/hosts/rig-timers.md` | (via timer) | `foss-setup/configs/host/rig/music-mirror/nas-music-mirror.service` |
| `nas-music-mirror.timer` | rig | daily ALAC mirror | `*-*-* 05:30:00 America/New_York`, `RandomizedDelaySec=300`, `Persistent=true`. See `reference/hosts/rig-timers.md` | `foss-setup/configs/host/rig/music-mirror/nas-music-mirror.timer` |
