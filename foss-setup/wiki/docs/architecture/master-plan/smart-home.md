...
- **HA backups — concrete method + live status:** Settings → System → **Backups**. **✅ Live (queue item 02, `ha-11`):** a dedicated least-priv Synology SMB user `ha-backup` + Supervisor CIFS mount `nas_backups` → `//192.168.10.4/backups` (agent `hassio.nas_backups`); **daily 04:45 automatic ENCRYPTED backups to both the local eMMC agent and the NAS, retention 3**. Validated a triggered backup landed encrypted on the NAS as a listable 7-member archive (a real restore path). Key in vault `hosts.ha.backup_password` (+ Proton Pass). Also separately back up the Midea `.json` token files.
