...
| qBittorrent → **Deluge** + Sonarr/Radarr/Prowlarr/Bazarr + sync agent | Managed seedbox — **Bytesized** (off-site) | Keeps all P2P off the home network. **Corrected model:** the seedbox ("Betty") runs **only Deluge + slskd**; the full *arr suite runs **on the NAS** — see Section 2 (`replacements.md`). |
