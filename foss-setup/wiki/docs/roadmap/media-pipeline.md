...
| `fix-49` | Navidrome total library wipeout: bad .ndignore written by the fix-28 guard + one empty-root CIFS scan flagged all 3495 tracks missing | ⬜ open | 1-3 hrs |
| `fix-50` | Bitmagnet DHT junk-grab storm: arrs re-grabbing owned titles hourly, foreign-audio junk imported into the library today; indexer also timing out via Prowlarr | ⬜ open | 1-3 hrs |
| `fix-54` | Movies/TV import stall: torrent payloads deleted under live torrents, move_completed wedge, 3-4 day arr retry loops, sample files back in the library | ⬜ open | 1-3 hrs |
