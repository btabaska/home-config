# Roadmap — gaming

26 task(s). Status mirrors `docs/progress.json` (the source of truth).

| Task | Title | Status | Effort |
|---|---|---|---|
| `fix-34` | Fix AMP scheduled backups (silently failing) + backup-bloat in restic + playit UDP churn | ✅ done | 1-3 hrs |
| `game-01` | Install LinuxGSM light co-op servers on the Mac mini (always-on) | 🗑️ retired | 60-90 min |
| `game-02` | AMP game panel on rig — create first Minecraft instance (UI) | ✅ done | 30 min (web UI) |
| `game-03` | Run heavy game servers on the CachyOS rig | ✅ done | 30-60 min per server |
| `game-04` | Expose game servers to friends safely via Tailscale (no open ports) | 🗑️ retired | 20-30 min |
| `game-05` | Install & autostart Apollo on the CachyOS rig (NVENC, 4K/120 HDR) | ✅ done | 30-45 min |
| `game-06` | Set up Moonlight clients and pair in-home (same Trusted VLAN) | ✅ done | 20-30 min |
| `game-07` | Remote streaming over Tailscale — ensure DIRECT (not DERP) | ✅ done | 20-40 min |
| `game-08` | Wake-on-LAN (recovery tooling): enable on the rig NIC and send from Mac mini/laptop | ✅ done | 20-30 min |
| `game-09` | Rig idle-power tuning (24/7 baseline) | ⬜ open | 20-30 min |
| `game-10` | GPU power-limit + (optional) undervolt the 3090 Ti, persistent across reboot | ✅ done | 30-45 min |
| `game-11` | Headless display for Apollo (HDMI dummy plug + display-policy service) | ✅ done | 20-45 min |
| `game-12` | Save-game sync with Ludusavi + Syncthing | ✅ done | 30 min |
| `game-13` | GPU contention policy (one 3090 Ti shared by stream/servers/AI) | ✅ done | 15 min |
| `game-14` | Game launcher (Heroic/Lutris) + RomM retro library | ⏸️ deferred | 45 min |
| `retire-gaming-01` | Final verified archives of all game-server data (pre-retirement backup) | ✅ done | 30-60 min |
| `retire-gaming-02` | Stop & isolate all game servers (30-day grace, 4-7AM ET window) | ✅ done | 1-2 h |
| `retire-gaming-03` | Delete game-server stacks after 30-day grace (next 4-7AM ET window) | ⬜ open | 1 h |
| `retro-01` | NAS ROM library layout | ✅ done | 30 min |
| `retro-02` | RomM — self-hosted ROM manager | ✅ done | 60-90 min |
| `retro-03` | Save/state sync mesh (Syncthing) | ⬜ open | 60 min |
| `retro-04` | SteamDeck: EmuDeck/RetroDeck wiring | ⬜ open | 45 min |
| `retro-05` | Rig: emulation frontend + RomM integration | ✅ done | 60 min |
| `retro-06` | RetroAchievements + cheat DB | ⬜ open | 30 min |
| `retro-07` | Retro handheld onboarding (e.g. AYN Thor Max) | ⏸️ deferred | 45 min |
| `retro-08` | RomM RetroAchievements dashboard (view-only unlock %/hardcore stats) | ⬜ open | ~15 min config |

[← Roadmap overview](index.md)
