...
> **RESOLVED (fix-44, 2026-07-19):** verified end-to-end: `wiki-drift-check.sh` exits 0
> both locally and on mini against HEAD `d178975` ("wiki in sync"). The stale pages were
> regenerated by fix-29 (see NOTE below); fix-44 closes the finding and adds the sibling
> `tracker-integrity` check for the tracker-source layer (see M46).
...
> **RESOLVED (fix-44, 2026-07-19):** took the "add the definition" path — retroactive
> `nas-00e` task written into tasks.json (nas-foundation, after nas-00d; the work was
> real and is live: mini automounts `//nas/music` at `/mnt/nas/music` ro + `/mnt/nas-music-rw`
> rw, rig at `/mnt/nas-music-ro`). The stale `_meta.task_count/completed_count` fields were
> REMOVED instead of corrected (operator decision): no code consumer since the HTML tracker
> retired (f1bb884) and they had drifted to 234/172 vs a reality of 269/188 — todo.md + the
> wiki roadmap are now the only published counts. Guard: `tracker-integrity` check
> (checks.d/git-hygiene.yaml, scripts/verification/tracker-integrity.py) — orphan/duplicate/
> contradictory status ids and any _meta count-field reintroduction fail the daily sweep.
...
> **RESOLVED (fix-44, 2026-07-19):** the page was regenerated in fix-29's full `gen-*.py`
> flush; verified now: "subtree"/SPLIT_SHA survive only inside the script's own HISTORY
> comment (accurate prose, not stale topology), and `wiki-drift-check.sh` is green locally
> and on mini at HEAD `d178975`. Recurrence of the same-commit class is guarded by the
> `wiki-drift` daily check (wiki-05).
...
> **RESOLVED 2026-07-19 (fix-42).** `roles/backup` rewritten to deploy the live design **verbatim from `scripts/backup/*`** (single source of truth, SOPS gate dropped): pinned restic 0.19.1 → `/usr/local/bin` on Debian with the apt 0.12 footgun **removed**, script/units/timer/ntfy/dead-man drop-in/excludes/verification-wrappers/sudoers all role-owned; only `/etc/restic/env` stays hand-seeded (runbook `backup-restore.md`). Fleet alerting parity restored: the rig regained `ntfy-notify@` + its healthchecks dead-man — `restic-backup-rig` had been "down" since the fix-20 rebuild silently lost the drop-in, the exact reproducibility failure this finding predicted. The dead-man URL now comes from `RESTIC_HC_URL` in the secret env via a verbatim drop-in (no more unmirrored hand-edit on mini). Guarded by `restic-role-matches-source-{mini,rig}` + `ansible-site-converged-mini` + `ansible-pull-ok-rig` (checks.d/backups.yaml).
...
> **RESOLVED (fix-44, 2026-07-19):** the 2026-07-15 red was cleared by fix-29's regen; the
> re-red in TODAY's 10:18 sweep was a commit race — the sweep tested HEAD `7600ea7` (fix-43
> junk cleanup, which added 2 scripts) 23 minutes BEFORE the same-session regen commit
> `d178975` (10:41) was pushed. Re-ran the exact check cmd on mini after fast-forwarding the
> cache clone to `d178975`: exit 0, "wiki in sync" — next daily sweep goes green with no
> action. The triage-404 side-issue was fixed separately (fix-30 class).
