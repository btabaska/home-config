...
- **OPEN (user approval pending): the Sonarr fixes** from this session's earlier diagnosis — unpackerr silent-hang restart (6 rar'd queue items), Archer S04 season-pack manual import (13 queue rows), Prowlarr IPT query limit, second TV indexer. Sonarr left untouched by request.
