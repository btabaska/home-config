...
### Lights → HA → Apple Home: HomeKit Bridge created, awaiting user pairing (2026-07-13)

Scope: get existing lights onto HA and bridge them to Apple Home for Siri/HomePods — **existing hardware only, no purchases** (Zigbee backbone stays excluded). Worked in an isolated worktree/branch (`ha-device-integration-apple-home-4b702f`) to avoid stomping Run 5. All against the LIVE HA (`192.168.10.50`, core 2026.6.4, state RUNNING).

- **Lights verified working (ha-05 reality-done).** Hue (~71 entities) + 2× Elgato already integrated. Proved bidirectional HA→Hue→physical control with a **state-preserving round-trip on `light.attic_attic`** (on→off→on, restored to br=255). 73 light entities total; live states drift with wall switches (~23 `unavailable` = powered off at the wall — a Hue reality, not a fault).
- **Registry cleanup:** the 2 Elgato key lights were unassigned → homed to **Office** (device-level `area_id=office`, per research/11). Light→area now: 50 in clean canonical areas; **21 still sit in 5 `Legacy —` areas** (Basement 11, Guest Bedroom 2, Lounge 2, Master Bathroom 3, Upstairs Bathroom 3) awaiting the user's answers to research/11 §open-questions before re-homing.
- **HomeKit Bridge ADDED** (`homekit` config entry `HASS Bridge:21064`, entry_id `01KXDTZPAWXB7P57PDXCN13J7M`, `state=loaded`, scoped to `include_domains=[light]`, 73 accessories, under the 150 limit). Pairing code in the HA "HomeKit Pairing" notification (consumed on pair; regenerates if re-paired). **User must scan/enter on their iPhone** — pairing is the only step that commits.
- **NO network change needed / no Run-5 sign-off required.** HA (Trusted `.50`) advertises `_hap._tcp` on Trusted; per research/11 the iPhone + HomePods live on Trusted too, so mDNS discovery is native — the charter's cross-VLAN mDNS gotcha does not apply. Caveat: could NOT independently confirm the iPhone's live subnet (its SSID/BSSID/connection sensors read `unavailable`; device_tracker is GPS). Fallback stated to user: if Apple Home can't find the bridge, the Apple gear is on a different VLAN/SSID → becomes a UniFi mDNS-reflection item (Run-5 territory).
- **Double-exposure warning surfaced (not silently picked):** Hue bridge v2 is HomeKit-native and may already be in the user's Apple Home directly; pairing the HA bridge on top would duplicate every light. Recommended the HA-bridge path (one control plane + HA automations) and gave the user "remove direct Hue from Apple Home first" steps. Their call.
- **⚠️ Backups are effectively ABSENT (new live-state finding).** Only agent = `hassio.local` (eMMC); no schedule, no NAS agent, no encryption password (`backup/config` all empty; 0 backups existed before this session). Created one local rollback snapshot (`pre-homekit-bridge-claude`, id `bdcf6693`) before the config change. The NAS-agent + Bitwarden-key backup task is unstarted — flagged HIGH.
- **HACS is NOT installed** → blocks `midea_ac_lan` and `emporia_vue` (both HACS). **No Midea account creds in the vault** either (Midea also needs a cloud handshake). Vault DOES have creds for UniFi Protect / Roborock / VeSync / Withings (all core integrations, ready to add on request — deferred, not lights).
- No commits made (user hasn't asked). Uncommitted on this branch: `wiki/docs/hosts/home-assistant.md` (integrations + backup status → reality) and this entry.

### SESSION CLOSE — DOCUMENTATION DEBT AUDIT complete (live stack = source of truth) (2026-07-10)

**Mandate**: full audit of ALL documentation with the LIVE service stack as the single source of truth; fix docs to match reality (not vice-versa); live-state defects become tasks, not doc edits. Cold-start — everything verified with read-only probes before writing. **Every claim below was checked against the running fleet.**

**Committed (home-config, GitHub `origin`): 80efbaa · e39ef82 · e43275b · 2fcfe29 · fca010d + handoff.** **Mirror commits (mini `/opt/stacks`, pushed to forgejo): 87577d0 · 02364a2 · 9681f39.** **Wiki rebuilt `--strict` (0 broken links) + deployed to `/opt/stacks/wiki/site`, verified serving 200 incl. the corrected rig facts.** Sweep at close: 73/74 pass (1 warn = `playit-bedrock-public`, the known intermittent upstream claim-degradation, manual 3/4 pings pass — not a regression).

**DRIFT INVENTORY (was → now):**
- **The "LiteLLM on the mini" phantom — excised everywhere.** LiteLLM/Ollama/Open WebUI/mcpo ALL run on the **rig** (compose in the separate `local-ai-tooling` repo, not `foss-setup`). Nothing references mini:4000 (verified). Fixed in: `wiki/hosts/rig.md`, `wiki/hosts/mini.md`, `wiki/hosts/home-assistant.md`, `wiki/runbooks/wake-the-rig.md`, `service-catalog.yaml`, `stacks/litellm/compose.yaml` header, `configs/gaming/server-guide.md`. **The rig AI surface has NO fallback** (the planned mini fallback never deployed) — stated plainly now (was described as "resilience fallback keeps AI working").
- **`checks.d/rig.yaml` / `verification/README.md` / `pcie-aer/README.md` "mini→rig SSH blocked by tailnet ACL"** → FALSE (verified `RIG-SSH-OK`). Corrected: rig checks are HTTP-by-design (test the endpoints clients hit); pcie-aer is rig-local because AER/SMART need local sysfs. **Only mini→seedbox SSH is genuinely ACL-blocked** ("tailnet policy does not permit") — that check stays disabled, its reason is correct, left as-is.
- **NAS RAM: docs said "4 GB, 20 GB upgrade pending"** → verified **20 GB live** (`free -m`). Fixed `wiki/hosts/nas.md` (limp-mode rule rewritten), `server-guide.md`, `media-automation/README.md` (OOM risk was 4 GB-premised).
- **ansible-pull "not yet deployed on rig" / gpu-power-tune "fails at boot"** → both LIVE and green (verified: timers active, dead-manned; gpu service exit 0, 300 W). While here, **deployed the hardened `gpu-power-tune.sh` from the repo to the rig** (on-disk copy was the stale pre-fix one; restarted service, verified 300 W + persistence). Fixed `wiki/hosts/rig.md`, `wake-the-rig.md`, `operations/ansible.md`.
- **Backups: `backup-restore.md` said "B2 does not exist yet, restic pending"** → restic→B2 is **LIVE on mini+rig, dead-manned**; immich pg_dump live+dead-manned. Real restic-restore commands + `restore-test.sh` replace the stub. NAS-Tier-1-Hyper-Backup→B2 correctly remains the one true pending item (nas-02).
- **DNS chain "not yet healthy" (dns-02/03 pending)** → healthy + guarded (both closed, probed hourly, auto-reopen). Fixed `network.md`, `gateway.md`, `dns-outage.md`, `checks.d/dns.yaml` header. Drill dns-04 still genuinely never run.
- **Seedbox "tailnet-ACL blocked, unverifiable"** → `ssh seedbox` works; +deluge-reaper cron documented; liveness note replaces the warning.
- **Kuma "50 monitors / daily-only sweep" / verification "being built"** → 53 Kuma (incl. keyword/functional), two-tier sweep (74 checks) LIVE; reopen-suggestions (verify-05) live. Fixed `runbooks/verification.md` (real schema, real result paths, fictional `--only` flag removed, root-run warning), `operations/tracker.md`.
- **Retired SBOM feature still described as live** in the ansible `sbom` role header, `ansible/README.md` tree, `group_vars/all.yml`, and `restore-runbook-template.md` (a rebuild-from-template would have redeployed a dead service) → all bannered RETIRED / "don't reinstall".
- **Generator (`gen-wiki-services.py`) was structurally wrong** — it never loaded the list-schema `service-catalog.yaml` (silently fell back to built-ins), dropped REMOVED/NOT-DEPLOYED banners on regen, and didn't walk `configs/gaming` (rig) or `configs/git` (forgejo). **7 running services had no wiki page at all** (amp, palworld, playit, forgejo, romm, bedrock-connect, bgutil-pot) — now generated. `service-catalog.yaml` backfilled with all live services (gaming stack, NAS workers, recyclarr cron).
- **`maintainerr` Caddy vhost was still live and 502-ing** (service removed from plan 07-08) → vhost commented out, caddy reloaded (mini commit 02364a2). Repo mirrors: re-pinned `libreseerr` to the actually-running digest (07-07 pin was never deployed), synced stale `tdarr/maintainerr/litellm/miniflux/wallabag` mirrors, **untracked 3 secret files** that were committed in mini `/opt/stacks` (`ntfy/.healthchecks-token`, `ntfy/.phone-password`, `vaultwarden/.env.bak-tokenhash`) + added .gitignore rules (87577d0).
- **Tracker vs reality**: closed **game-03** (AMP+Palworld live on rig, public paths protocol-ping verified — was reality-done/tracker-open) → 140/223. Marked todo-guide TASK 01/04/05 done (ntfy-confirmed / TMDb-vaulted+wired / RomM-admin-vaulted+login-verified). No other reality-vs-tracker mismatch found; reopen-suggestions empty.

**NEW live-state findings (reality, not doc, issues — NOT papered over):**
- **AI-stack single-point-of-failure**: rig down = the entire AI surface is down with no fallback. Not a doc bug (docs now say so); flagging as a resilience gap — either deploy the mini `litellm` fallback (decision #2) or accept it.
