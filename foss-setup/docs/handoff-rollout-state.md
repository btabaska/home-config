...
- **STILL RECOMMENDED (needs user, network change / lockout risk)**: give mini a **static IP** (192.168.10.2/24) in netplan + a **UniFi DHCP reservation** for `98:5a:eb:ca:b2:ef`, eliminating the lease/renewal dependency entirely. Also worth: figuring out why UniFi ignores renewals (controller-side). Apply in the 4-7AM ET window or with user present; `netplan try` auto-reverts.
- **Secondary (separate) issue seen in the same logs**: `sbom-nightly.service`/`syft` OOM-killed Jul-08 04:14 (6.5GB RSS on an 8GB box; Committed_AS ~16GB = 2× overcommit). Not the outage cause, but memory pressure on this 2014 dual-core/8GB Mac mini running ~40 containers is a standing risk — consider capping syft memory / MemoryMax on heavy timers.
