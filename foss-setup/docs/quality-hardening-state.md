...
**Done:** #1 Pinchflat→Plex · #2 Libreseerr→CWA · #3 MusicSeerr · #5 acceptance-test framework design doc (`wiki/docs/runbooks/acceptance-testing.md`) · #6 movie/TV→Plex acceptance checks (2026-07-14) · #7 book acceptance test · #8 album acceptance test · #9 pinchflat acceptance test · #10 done-audit (2026-07-14).

**#6 + #10 — DONE 2026-07-14** (Plex drained/recovered — `pinchflat-plex-visible` back to 1364/1364 ratio 1.00). **#6:** new primitive `arr-plex-journey.py` + checks `radarr-movies-in-plex` / `sonarr-tv-in-plex` (crit, min-ratio 0.85) — match Radarr/Sonarr items-with-files to Plex by **external id** (tmdb/tvdb), immune to item≠file count; negative-tested 3 ways (wrong section, bad key, ratio-below-threshold). Both pass live (movies 0.939, TV 0.962). **#10 done-audit:** full media suite **13/13 green after remediation**; surfaced two real defects liveness never showed — (a) **movie data-quality gap:** 12/197 movies unwatchable (**7 sample-file imports**, 2 `.iso`, 1 wrong-file map, 2 Plex-mismatch) → remediation backlog below; (b) **MusicSeerr phantom recurred** (3OH!3 album unmonitored in Lidarr) → **fixed** (monitor + AlbumSearch album 6037, check now `PHANTOM_OK`). "Done requires an e2e pass" rule recorded in the acceptance-testing runbook. Memory [[plex-bulk-ingest-analysis-storm]], [[arr-sample-file-imports]].

**Movie watchability remediation backlog (2026-07-14, needs decision):** 7 movies where Radarr imported a junk **sample file** (`hasFile=True` but a 6–24 MB `sample.*`): American Hustle, Take Me to the River, Spider-Man: Homecoming, Intruders, Rampage, xXx: Return of Xander Cage, American Reunion. Fix = delete the sample movieFile + re-search each (slow/indexer-dependent, so not auto-run). Plus 2 `.iso` disc images Plex won't play (Scooby WrestleMania, Bodies Bodies Bodies) and 1 wrong-file map (All About My Mother ← Mamma Mia file). Left for a remediation pass.
...
- **Plex 503 (RESOLVED 2026-07-14):** the §4/YouTube metadata-analysis storm drained on its own (1026→730→3 live) as the user chose; Plex serves 200 again and `pinchflat-plex-visible` is back to 1364/1364. This unblocked #6/#10 (both now done). Diagnosis retained in memory [[plex-bulk-ingest-analysis-storm]]; framework anti-pattern ("producer flood → consumer storm") in the acceptance-testing runbook.
