...
- **sbom-nightly (mini)**: three stacked root causes found and fixed for "Dependency-Track is empty": (1) stale deployed script without SCAN_EXCLUDES → nightly OOM (excludes deployed, scan now completes); (2) upload curl forced a `Content-Type: multipart/form-data` header without a boundary → every upload 415 (header removed); (3) `PUT /api/v1/bom` expects JSON, multipart needs `POST` (switched). Both hosts redeployed. REMAINING HUMAN GATE: the API key lacks the PROJECT_CREATION_UPLOAD permission — uploads now return 401 "does not have permission to create project". Grant it in the D-Track admin UI (admin password is NOT in the vault; default admin/admin already rejected — add creds to vault as `dtrack.admin_*`). Next timer run (03:35 UTC nightly) populates the portfolio once granted.
