# Google Nest in Home Assistant — SDM / OAuth Setup Checklist
...
> current Home Assistant version before starting. Steps below reflect the 2026 flow.
...
- [ ] Home Assistant reachable over **HTTPS** at a public/known URL, OR you will use
      the My Home Assistant redirect (`https://my.home-assistant.io/redirect/oauth`).
      The OAuth redirect URI must match exactly later.
...
breaks. For a fully-local thermostat, consider Zigbee/Z-Wave or an ecobee-class device
in the future — but that's out of scope for keeping the existing Nest hardware.
