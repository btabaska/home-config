...
| **Palworld** | Yes | No | No | Yes | **CachyOS** (24/7 rig) | Both | 24/7 (rig) |
...
- **Verdict:** Yes
- **Can host on:** Mac mini **No** · NAS (20 GB) **No** · CachyOS **Yes**
- **Recommend:** **CachyOS** only (24/7 rig) — CPU- and RAM-heavy; plan Section 4 explicitly routes Palworld to the rig.
- **Reach:** Both
- **Uptime:** 24/7 (rig)
...
The rig is 24/7 since 2026-07-08 — WoL is no longer part of the game-night
workflow. Keep it working as RECOVERY tooling for a power outage or accidental
shutdown (rig down = incident):

1. Keep WoL enabled in BIOS on the CachyOS rig (`scripts/gaming/enable-wol-cachyos.sh`).
2. To recover a downed rig, send a magic packet from phone/laptop, HA automation, or SSH: `wol <mac-address>` (or `scripts/gaming/wake-rig.sh`).
3. Game servers restart via systemd/LinuxGSM after the recovery boot (ansible-pull or manual).
