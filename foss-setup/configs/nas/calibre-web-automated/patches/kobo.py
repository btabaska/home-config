...
def generate_sync_response(sync_token, sync_results, set_cont=False):
    ...
    if config.config_kobo_proxy and not set_cont:
        ...
        try:
            ...
            # fix-57 SM32 (2026-08-02): the upstream Kobo store returns an ERROR
            # OBJECT ({"ResponseStatus": {...}}) instead of a sync-item LIST when
            # the forwarded request lacks valid Kobo-store device auth (expired /
            # never-authenticated device, or a non-device sync probe). Upstream
            # did `sync_results += store_sync_results` unconditionally, so `+=`
            # iterated the dict's KEYS and injected the bare string 'ResponseStatus'
            # into the device sync payload (contaminated BOTH admin + kobo2). Only
            # merge a real list; surface the store failure in the log instead of
            # poisoning the array. Shadow-patched file, pinned to image
            # ghcr.io/new-usemame/calibre-web-nextgen:v4.0.7 — RE-DERIVE on any CWA
            # image bump (guarded by check cwa-image-digest-pinned + the
            # all-dicts assertion in cwa-kobo-sync-consumer).
            if isinstance(store_sync_results, list):
                sync_results += store_sync_results
            else:
                log.warning(
                    "Kobo store sync returned a non-list (%s) — not merging; "
                    "store leg likely erroring (missing/expired device store auth): %.200s",
                    type(store_sync_results).__name__, str(store_sync_results),
                )
